...
func TestGet(t *testing.T) {
	...
	self := NewGroup("scores", 2<<10, GetterFunc(
	...
	for k, v := range db {
		if view, err := self.Get(k); err != nil || view.String() != v {
			t.Fatal("failed to get value of Tom")
		}
		if _, err := self.Get(k); err != nil || loadCounts[k] > 1 {
			t.Fatalf("cache %s miss", k)
		}
	}

	if view, err := self.Get("unknown"); err == nil {
